...
import {addNewNetworkTemplate} from "../../../../redux/actions/project";


export const ImportNetworksTemplatesDialog = ({setDialogIsOpened, dialogIsOpened}) => {
    ...
    const [userNetworkTemplatesData, setUserNetworkTemplatesData] = useState([])
    ...
    useEffect(() => {
        if (userProjects) {
            ...
            let allUserNetworkTemplates = []
            otherProjects.forEach(userProject => {
                const {templates: {networks}} = userProject
                const networksTemplatesWithProjectName = networks.map(template => ({...template, projectName: userProject.info.name}))
                allUserNetworkTemplates = [...allUserNetworkTemplates, ...networksTemplatesWithProjectName]
            })

            setUserNetworkTemplatesData(allUserNetworkTemplates)
        }
    }, [userProjects])
    ...
    const getNetworkTemplateName = (selectedUserDataItem) => {
    ...
    const renderUserDataItem = (item) => {
        return (
            <MenuItem
                ...
                text={getNetworkTemplateName(item)}
            />
        );
    }
    ...
    return <Dialog
        ...
        title={<span className={styles.dialogTitle}>Import network template</span>}
        autoFocus={false}
        enforceFocus={false}
    ...
    >
        <div className={[Classes.DIALOG_BODY]} style={{overflow: "hidden", paddingRight: 4, paddingLeft: 4}}>
            ...
            <p className={styles.dialogText}>
                Select network template to import:
            </p>
            <Select
                items={userNetworkTemplatesData}
                itemRenderer={renderUserDataItem}
                activeItem={selectedUserDataItem && getNetworkTemplateName(selectedUserDataItem)}
                className="fullwidth"
            ...
            >
                <Button text={<span
                    className={styles.selectText}>{selectedUserDataItem && getNetworkTemplateName(selectedUserDataItem) || "Select template..."}</span>}
